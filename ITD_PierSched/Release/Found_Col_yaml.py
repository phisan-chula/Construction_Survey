...
def CalcColumns( PIER, CHOS ):
    ...
    _,rot = divmod( 90.-final_az , 360 )
    col_pos = list()
    for i,[chain,offset] in CHOS.items():
        rot_col = rotate( Point(chain,offset) , rot, 
                        use_radians=False, origin=Point(0.0,0.0) )
        rot_trn_col = translate( rot_col ,
                       xoff=PIER['EASTING'], yoff=PIER['NORTHING'] ) 
        col_pos.append( (PIER['PIER NO.'], PIER['FOUNDATION TYPE'], 
                          i, final_az, rot_trn_col ) )
    df =  pd.DataFrame( col_pos, columns=['PierNum','PierTyp',
...
for i,row in df_pier.iterrows():
    ...
    if pd.isna( ftype ):
        ...
        hole_EN = CalcColumns( row, chos ) 
        if df_column is None:
            df_column = hole_EN.copy()
        else:
            df_column = pd.concat( [df_column, hole_EN], 
